...
const inputCls = 'w-full rounded-xl border border-[#3f3f46] bg-[#0f0f13] px-3.5 py-2.5 text-sm text-[#fafafa] placeholder:text-[#52525b] outline-none transition focus:border-[#3b82f6]/60 focus:ring-2 focus:ring-[#3b82f6]/20 box-border resize-none';
const labelCls = 'mb-1.5 block text-xs font-medium text-[#a1a1aa]';

const MetaRow = ({ label, value }) => (
  <div className="flex items-start gap-3 py-2.5 border-b border-[#3f3f46]/40 last:border-0">
    <span className="w-28 shrink-0 text-xs text-[#71717a]">{label}</span>
    <span className="text-sm text-[#fafafa] break-words">{value || '—'}</span>
  </div>
);
...
const GradeCard = ({ ticket }) => {
  ...
  return (
    <div className="rounded-2xl border bg-[#18181b] overflow-hidden" style={{ borderColor: `${cfg.color}30` }}>
      {/* Grade header */}
      <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: `${cfg.color}20`, backgroundColor: `${cfg.color}08` }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#71717a]">{cfg.desc}</p>
          </div>
        </div>
      ...
      <div className="px-5 pt-4 pb-1">
        <div className="h-2 overflow-hidden rounded-full bg-[#3f3f46]/40">
      ...
      {ticket.ai_grade_feedback && (
        <div className="px-5 py-3.5">
          <p className="text-xs font-medium text-[#a1a1aa] mb-1.5">AI Feedback</p>
          <p className="text-sm leading-relaxed text-[#fafafa]">{ticket.ai_grade_feedback}</p>
        </div>
      )}
  ...
  if (!ticket) return (
    <div className="flex min-h-screen items-center justify-center">
      <div className="text-center">
        <p className="text-[#71717a]">Ticket not found.</p>
  ...
  return (
    <div className="min-h-screen px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-350 space-y-6">
        ...
        {/* Back + breadcrumb */}
        <div>
          <Link to="/tickets" className="inline-flex items-center gap-1.5 text-sm text-[#71717a] transition hover:text-[#fafafa]">
        ...
        <div className="flex flex-wrap items-start gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <span className="text-xs font-mono text-[#3f3f46]">#{ticket.id}</span>
            ...
            <h1 className="text-xl font-bold text-[#fafafa] leading-snug">{ticket.title}</h1>
          </div>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-5">
            <div className="rounded-2xl border border-[#3f3f46]/60 bg-[#18181b] p-5">
              <p className="mb-3 text-[10px] font-semibold uppercase tracking-widest text-[#52525b]">Details</p>
            ...
            <div className="rounded-2xl border border-[#3f3f46]/60 bg-[#18181b] p-5">
              <p className="mb-3 text-[10px] font-semibold uppercase tracking-widest text-[#52525b]">Description</p>
              <p className="text-sm leading-relaxed text-[#a1a1aa] whitespace-pre-wrap">{ticket.description || '—'}</p>
            </div>

            {/* Resolution write-up (read-only if already submitted) */}
            {hasGrade && ticket.resolution_steps && (
              <div className="rounded-2xl border border-[#3f3f46]/60 bg-[#18181b] p-5 space-y-4">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-[#52525b]">Resolution Write-up</p>
                <div>
                  <p className="mb-1 text-xs font-medium text-[#a1a1aa]">Steps Taken</p>
                  <p className="text-sm leading-relaxed text-[#fafafa] whitespace-pre-wrap">{ticket.resolution_steps}</p>
                </div>
                <div>
                  <p className="mb-1 text-xs font-medium text-[#a1a1aa]">Root Cause</p>
                  <p className="text-sm leading-relaxed text-[#fafafa] whitespace-pre-wrap">{ticket.root_cause}</p>
                </div>
                <div>
                  <p className="mb-1 text-xs font-medium text-[#a1a1aa]">Solution Applied</p>
                  <p className="text-sm leading-relaxed text-[#fafafa] whitespace-pre-wrap">{ticket.solution_applied}</p>
                </div>
              </div>
            )}
          </div>
            ...
            {ticket.ai_priority_suggestion ? (
              <div className="rounded-2xl border border-[#a78bfa]/20 bg-[#a78bfa]/5 p-5">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-[#71717a]">Suggested Priority</span>
                    <Pill label={ticket.ai_priority_suggestion} map={PRIORITY_CLS} />
                  </div>
                  {ticket.ai_category && (
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-[#71717a]">Category</span>
                      <span className="text-xs font-medium text-[#fafafa]">{ticket.ai_category}</span>
                    </div>
                  )}
                  {ticket.ai_recommendation && (
                    <div className="pt-1 border-t border-[#a78bfa]/15">
                      <p className="mb-1 text-xs text-[#71717a]">Recommendation</p>
                      <p className="text-sm leading-relaxed text-[#a1a1aa]">{ticket.ai_recommendation}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-[#3f3f46]/60 bg-[#18181b] p-5">
                <div className="mb-2 flex items-center gap-2">
                  <span className="text-[#a78bfa]"><SparkleIcon /></span>
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-[#52525b]">AI Triage</p>
                </div>
                <p className="text-xs text-[#3f3f46]">No AI analysis available.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="border-t border-[#3f3f46]/60 pt-6">
            <h2 className="text-base font-semibold text-[#fafafa]">Resolution</h2>
          </div>
          ...
          {ticket.requires_admin_approval && (
            <div className="rounded-2xl border border-[#ef4444]/25 bg-[#ef4444]/8 px-5 py-4 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-[#fafafa]">Admin approval required</p>
                  <p className="text-xs text-[#71717a]">Resolution scored below 60 — an admin must review and approve before this ticket closes.</p>
                </div>
              </div>
              {isAdmin && (
          ...
          {/* Resolution form */}
          {showForm && (
            <div className="rounded-2xl border border-[#3f3f46]/60 bg-[#18181b] p-6 space-y-5">
              <div>
                <h3 className="text-sm font-semibold text-[#fafafa]">Submit Resolution Write-up</h3>
                <p className="mt-0.5 text-xs text-[#71717a]">Claude will grade your resolution and automatically determine the ticket outcome.</p>
              </div>
          ...
          {alreadyResolved && !hasGrade && (
            <div className="rounded-2xl border border-[#3f3f46]/60 bg-[#18181b] px-5 py-4">
              <p className="text-sm text-[#71717a]">This ticket was resolved without a formal resolution write-up.</p>
            </div>
          )}
          ...
          {hasGrade && ticket.requires_admin_approval && !isAdmin && (
            <div className="rounded-2xl border border-[#3f3f46]/60 bg-[#18181b] px-5 py-4">
              <p className="text-sm text-[#71717a]">Waiting for an admin to review and approve this resolution.</p>
            </div>
          )}
        </div>
